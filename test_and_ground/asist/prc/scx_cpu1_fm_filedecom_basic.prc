PROC scx_cpu1_fm_filedecom_basic
;*******************************************************************************
...
s ftp_file (uploadDir, testFileCompressed, testFileCompressed, hostCPU,"P")
wait 5
...
s scx_cpu1_fm_fileinfodisplay (fullOriginalTestFileName)
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_InfoFileName = fullOriginalTestFileName) THEN
...
s scx_cpu1_fm_fileinfodisplay (fullTestFileUncompressed1)

wait until ((SCX_CPU1_FM_CMDPC = cmdctr) OR (SCX_CPU1_FM_CMDEC = errcnt)) timeout 20
...
/SCX_CPU1_FM_Decompress File=fullOriginalTestFileName DestName=fullTestFileUncompressed1
...
if ( cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_InfoFileName = fullTestFileUncompressed1) THEN
...
s scx_cpu1_fm_fileinfodisplay (fullTestFileUncompressed2)
...
/SCX_CPU1_FM_Decompress File=fullOriginalTestFileName DestName=fullTestFileUncompressed2
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_InfoFileName = fullTestFileUncompressed2) THEN
...
/SCX_CPU1_TST_FM_Open File =fullOriginalTestFileName

wait until ((SCX_CPU1_TST_FM_CMDPC = cmdctr) OR (SCX_CPU1_TST_FM_CMDEC = errcnt)) timeout 20
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (p@SCX_CPU1_FM_FileStatus = "Open") THEN
    ...
    write "<!> Failed - Expected file status 'Open'. Rcv'd '", p@SCX_CPU1_FM_FileStatus, "'"
  endif
else
  write "<!> Failed (1003;2011) - File Info Command Failed."
  ut_setrequirements FM_2009, "F"
...
if (SCX_CPU1_FM_NumOpen = expOpenFiles) THEN
  write "<*> Passed - The number of open files is correct: ",SCX_CPU1_FM_NumOpen
else
  write "<!> Failed - Expected ",expOpenFiles, " open files. TLM reports ",SCX_CPU1_FM_NumOpen
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_DECOM_SRC_OPEN_ERR_EID, "ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_Decompress File=fullOriginalTestFileName DestName=testDir2 & "/openSrc.tst"
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Received EM ", SCX_CPU1_find_event[1].eventid, "; Expected ",FM_DECOM_SRC_OPEN_ERR_EID
...
/SCX_CPU1_TST_FM_Close File=fullOriginalTestFileName
...
if (SCX_CPU1_TST_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "  Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", TST_FM_CLOSEFILE_INF_EID, "."
  endif
else
...
write ""
write ";*********************************************************************"
write ";  End procedure scx_cpu1_fm_filedecom_basic                          "
write ";*********************************************************************"
ENDPROC
