...
write ";*********************************************************************"
ut_setupevents "$SC", "$CPU", {FMAppName}, FM_SET_PERM_OS_ERR_EID, "ERROR", 1

errcnt = $SC_$CPU_FM_CHILDCMDEC + 1

; Sent the command
/$SC_$CPU_FM_SetFilePermissions File=fullTestFileName NewPerms=775
...
if (UT_TW_Status = UT_Success) then
  ...
  if ($SC_$CPU_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed - Event message ",$SC_$CPU_evs_eventid," received. Expected Event message ",FM_SET_PERM_OS_ERR_EID
  endif
else
