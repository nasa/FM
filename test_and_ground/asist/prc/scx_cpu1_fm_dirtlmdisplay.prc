proc scx_cpu1_fm_dirtlmdisplay (directoryName, offsetGiven, expResult, statFlag)
;==============================================================================
;
...
/SCX_CPU1_FM_DirListTlm DirName=directoryName Offset=offsetGiven GetStatFlag=statFlag
...
if (actualResult = "Pass") then
  ...
  if (writeToLog = 1) then
    if (SCX_CPU1_FM_DirOffset > SCX_CPU1_FM_TotalFiles) then
      numToDisplay = 0
    elseif ((SCX_CPU1_FM_TotalFiles - SCX_CPU1_FM_DirOffset) < maxFilesToDisplay) then
      numToDisplay = (SCX_CPU1_FM_TotalFiles - SCX_CPU1_FM_DirOffset)
    else
    ...
    write "  Directory Name        = ", SCX_CPU1_FM_DirName
    write "  Directory Offset      = ", SCX_CPU1_FM_DirOffset
    write "  Total Files in Dir    = ", SCX_CPU1_FM_TotalFiles
    write "  Files in Packet       = ", SCX_CPU1_FM_PktFiles
    FOR currentDis = 1 to numToDisplay DO
      if (currentDis = 1) then
        write ""
        write " File Listing: "
      endif
      write "  File " & currentDis & " Name          = ",SCX_CPU1_FM_DirList[currentDis].name
      write "  File " & currentDis & " Size          = ",SCX_CPU1_FM_DirList[currentDis].FileSize
      write "  File " & currentDis & " Last Mod Time = ",SCX_CPU1_FM_DirList[currentDis].LastModTime
      write "  File " & currentDis & " Permissions   = ",SCX_CPU1_FM_DirList[currentDis].FilePerms
    ENDDO
    write ""
  endif
endif
