PROC scx_cpu1_fm_fileinfo_basic
;*******************************************************************************
...
;    FM2013	Upon receipt of a Set File Permissions command, FM shall set the
;		command-specified file's permissions to the command-specified
;		mode.
...
local cfe_requirements[0 .. ut_req_array_size] = ["FM_1003", "FM_1004", "FM_2007", "FM_2007.1", "FM_2007.1.1", "FM_2008", "FM_2008.1", "FM_2011", "FM_2013", "FM_3000", "FM_4000", "FM_5000"]
...
s scx_cpu1_fm_fileinfodisplay (fullTestFileName)
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_InfoFileName = fullTestFileName) THEN
...
local expOpenFileCnt = SCX_CPU1_FM_NumOpen + 1

/SCX_CPU1_TST_FM_Open File=fullTestFileName
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_NumOpen = expOpenFileCnt) THEN
    write "<*> Passed (4000) - HK Telemetry Reports number of open files correctly: ", SCX_CPU1_FM_NumOpen
    ut_setrequirements FM_4000, "P"
  else
    write "<!> Failed (4000) - HK Telemetry Reports number of open files incorrectly: ", SCX_CPU1_FM_NumOpen
    ut_setrequirements FM_4000, "F"
  endif
else
  write "<!> Failed (1003;2011) - File Info Command Failed."
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> Passed (1004;2008.1) - Event message ", SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements FM_1004, "P"
    ut_setrequirements FM_20081, "P"
  else
    write "<!> Failed (1004;2008.1) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_DELETE_SRC_OPEN_ERR_EID
...
/SCX_CPU1_FM_DeleteAll DirName=testDir
...
if (SCX_CPU1_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_find_event[2].num_found_messages = 1) THEN
    ...
    write "<!> Failed (2007.1.1) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_DELETE_ALL_FILES_ND_WARNING_EID, "."
...
/SCX_CPU1_TST_FM_Close File=fullTestFileName
...
write ";*********************************************************************"
write ";  Step 3.6: Send the Set File Permissions Command. "
write ";  NOTE: This command will fail for FM 2.5.2.0 due to OSAL not having"
write ";        this implemented."
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_SET_PERM_OS_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_FM_CHILDCMDEC + 1

; Sent the command
/SCX_CPU1_FM_SetFilePermissions File=fullTestFileName NewPerms=775

ut_tlmwait SCX_CPU1_FM_CHILDCMDEC, {errcnt}
if (UT_TW_Status = UT_Success) then
  write "<*> Passed (2013) - Set File Permissions command failed as expected."
  ut_setrequirements FM_2013, "P"

  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> Passed - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  else
    write "<!> Failed - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_SET_PERM_OS_ERR_EID
  endif
else
  write "<!> Failed (2013) - Set File Permissions command passed when failure was expected."
  ut_setrequirements FM_2013, "F"
endif

wait 5

write ";*********************************************************************"
write ";  Step 3.7: Send File Info Command to Verify Test File permissions. "
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_FILE_INFO_CMD_EID, "DEBUG", 1
...
write ""
write ";*********************************************************************"
write ";  End procedure scx_cpu1_fm_fileinfo_basic                           "
write ";*********************************************************************"
ENDPROC
