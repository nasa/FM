...
/SCX_CPU1_FM_DirCreate DirName=testDir1

ut_tlmwait  SCX_CPU1_FM_CMDPC, {cmdctr}
if (UT_TW_Status = UT_Success) then
...
/SCX_CPU1_FM_DirCreate DirName=testDir2
...
write ";*********************************************************************"
local openFileCnt = SCX_CPU1_FM_NUMOPEN
write "** Number of currently open files = ",openFileCnt

s get_tbl_to_cvt (ramDirPhys, freeSpaceTblName, "A", "cpu1_fmfreespacetbl",hostCPU,fsindex)
wait 5
...
for i = 0 to FM_TABLE_ENTRY_COUNT-1 do
  if (SCX_CPU1_FM_FreeSpaceTBL[i].Name = ramDir) then
    ramDirIndex = i
    break
  endif
enddo
...
if (UT_TW_Status = UT_Success) then
  ...
  upper32Size = SCX_CPU1_FM_FreeSpacePkt[ramDirIndex].Upper32
  lower32Size = SCX_CPU1_FM_FreeSpacePkt[ramDirIndex].Lower32
  write "Free space on '",SCX_CPU1_FM_FreeSpacePkt[ramDirIndex].Name, "' is ", %hex(upper32Size)," and ",%hex(lower32Size)
else
...
write ";*********************************************************************"
write ";  Step 2.4: Upload the max number of open files + 2 files to CPU1."
write ";*********************************************************************"
local destFileName
...
for i = 1 to OS_MAX_NUM_OPEN_FILES DO
  ...
  if (i < switchCnt) then
    ...
    s ftp_file(uploadDir1, testFile, destFileName, hostCPU, "P")
    totalExpected1 = totalExpected1 + 1
  else
    ;; Upload the same Test file but with a different destination name
    write "Uploading file as '", destFileName, "' to '", uploadDir2, "'"
    s ftp_file (uploadDir2, testFile, destFileName, hostCPU, "P")
...
if (UT_TW_Status = UT_Success) then
  ...
  newUpper32Size = SCX_CPU1_FM_FreeSpacePkt[ramDirIndex].Upper32
  newLower32Size = SCX_CPU1_FM_FreeSpacePkt[ramDirIndex].Lower32
  write "Free space on '",SCX_CPU1_FM_FreeSpacePkt[ramDirIndex].Name, "' is ", %hex(newUpper32Size)," and ",%hex(newLower32Size)
else
  write "<!> Failed (1003;4001) - Get Free Space command not sent properly."
  ut_setrequirements FM_1003, "F"
  ut_setrequirements FM_4001, "F"
...
while (allFilesRetrieved = FALSE) do
  ...
  s scx_cpu1_fm_dirtlmdisplay (testDir1, fileOffset)
  ...
  write "*** Total Files in directory = ", SCX_CPU1_FM_TotalFiles

  if (fileOffset > SCX_CPU1_FM_TotalFiles) then
    filesToOpen = 0
    allFilesRetrieved = TRUE
  elseif ((SCX_CPU1_FM_TotalFiles - fileOffset) < FM_DIR_LIST_PKT_ENTRIES) then
    filesToOpen = SCX_CPU1_FM_TotalFiles - fileOffset
  else
    filesToOpen = FM_DIR_LIST_PKT_ENTRIES
  endif

  for j = 1 to filesToOpen do
    fullFileName = testDir1 & "/" & SCX_CPU1_FM_DirList[j].Name
    ut_setupevents "SCX", "CPU1", "TST_FM", TST_FM_OPENFILE_INF_EID, "INFO", 1

    cmdctr = SCX_CPU1_TST_FM_CMDPC + 1

    /SCX_CPU1_TST_FM_Open File=fullFileName

    ut_tlmwait  SCX_CPU1_TST_FM_CMDPC, {cmdctr}
    if (UT_TW_Status = UT_Success) then
    ...
    if (SCX_CPU1_num_found_messages = 1) THEN
      write "<*> Passed - Expected Event message ",SCX_CPU1_find_event[1].eventid, " received"
    else
      write "<!> Failed - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", TST_FM_OPENFILE_INF_EID, "."
    endif
  enddo
...
while (allFilesRetrieved = FALSE) do
  ...
  s scx_cpu1_fm_dirtlmdisplay (testDir2, fileOffset)
  ...
  for j = 1 to filesToOpen do
    fullFileName = testDir2 & "/" & SCX_CPU1_FM_DirList[j].Name
    ut_setupevents "SCX", "CPU1", "TST_FM", TST_FM_OPENFILE_INF_EID, "INFO", 1
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_OPEN_FILES_CMD_EID, "DEBUG", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1

/SCX_CPU1_FM_ListOpenFiles
...
if (UT_TW_Status = UT_Success) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1003) - Expected Event message ", SCX_CPU1_find_event[1].eventid," not received."
    ut_setrequirements FM_1003, "F"
  endif
else
...
write ";  open files."
write ";*********************************************************************"
openFileCnt = SCX_CPU1_FM_NUMOPEN
...
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_DELETE_ALL_FILES_ND_WARNING_EID, "INFO", 2

cmdctr = SCX_CPU1_FM_CMDPC + 1

/SCX_CPU1_FM_DeleteAll DirName=testDir2
...
if (SCX_CPU1_find_event[2].num_found_messages = 1) THEN
  write "<*> Passed (2007.1.1) - Event message ", SCX_CPU1_find_event[2].eventid, " received ( File Open ) as expected"
  ut_setrequirements FM_200711, "P"
else
  write "<!> Failed (2007.1.1) - Expected Event message ", SCX_CPU1_find_event[2].eventid," not received."
...
s scx_cpu1_fm_dirtlmdisplay (testDir2, 0)
...
write "*** Total Files in directory = ", SCX_CPU1_FM_TotalFiles
if (SCX_CPU1_FM_TotalFiles = totalExpected2) then
  write "<*> Passed (2007.1) - Directory Listing verifies that no files were deleted."
  ut_setrequirements FM_20071, "P"
else
  write "<!> Failed (2007.1) - Directory '", testDir2, "' contained ",SCX_CPU1_FM_TotalFiles, " files. Expected ",totalExpected2, " files."
...
s scx_cpu1_fm_dirtlmdisplay (testDir1, 0)
...
if (SCX_CPU1_FM_TotalFiles < 10) then
  filesToDelete = SCX_CPU1_FM_TotalFiles
endif

for j = 1 to filesToDelete do
  ut_setupevents "SCX","CPU1",{FMAppName},FM_DELETE_SRC_OPEN_ERR_EID,"ERROR", 1

  fullFileName = testDir1 & "/" & SCX_CPU1_FM_DirList[j].Name
  write "*** CMDEC = ",SCX_CPU1_FM_CMDEC
  errcnt = SCX_CPU1_FM_CMDEC + 1

  /SCX_CPU1_FM_Delete File=fullFileName
  ...
  if (UT_TW_Status = UT_Success) then
    ...
    if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
      write "<*> Passed (1004;2008) - Expected Event message ",SCX_CPU1_find_event[1].eventid, " received"
      ut_setrequirements FM_1004, "P"
      ut_setrequirements FM_2008, "P"
    else
      write "<!> Failed (1004;2008) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_DELETE_SRC_OPEN_ERR_EID
...
write "*** Total Files in directory = ", SCX_CPU1_FM_TotalFiles
if (SCX_CPU1_FM_TotalFiles = totalExpected1) then
  write "<*> Passed (2008.1) - Directory Listing verifies that no files were deleted."
  ut_setrequirements FM_20081, "P"
else
  write "<!> Failed (2008.1) - Directory '", testDir1, "' contained ",SCX_CPU1_FM_TotalFiles, " files. Expected ",totalExpected1, " files."
  ut_setrequirements FM_20081, "F"
endif
...
while (allFilesRetrieved = FALSE) do
  ...
  write "*** Total Files in directory = ", SCX_CPU1_FM_TotalFiles
  if (closeCnt = 0) then
    closeCnt = SCX_CPU1_FM_TotalFiles / 4
    closeCnt = closeCnt * 3
  ...
  for j = 1 to filesToClose do
    fullFileName = testDir1 & "/" & SCX_CPU1_FM_DirList[j].Name
    ut_setupevents "SCX", "CPU1", "TST_FM", TST_FM_CLOSEFILE_INF_EID, "INFO", 1

    cmdctr = SCX_CPU1_TST_FM_CMDPC + 1

    /SCX_CPU1_TST_FM_Close File=fullFileName

    ut_tlmwait  SCX_CPU1_TST_FM_CMDPC, {cmdctr}
    if (UT_TW_Status = UT_Success) then
      write "<*> Passed - Close File command for '", fullFileName, "' sent properly."
    else
      write "<!> Failed - Close File command for '", fullFileName, "'.CMDPC = ",SCX_CPU1_TST_FM_CMDPC,"; Expected ",cmdctr
    endif

    if (SCX_CPU1_num_found_messages = 1) THEN
      write "<*> Passed - Expected Event message ",SCX_CPU1_find_event[1].eventid, " received"
    else
      write "<!> Failed - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",TST_FM_CLOSEFILE_INF_EID
    endif
  enddo
...
if (SCX_CPU1_FM_NUMOPEN = (openFileCnt - closeCnt)) then
  write "<*> Passed - Open File count as expected"
else
  write "<!> Failed - Open File count = ",SCX_CPU1_FM_NUMOPEN, ". Expected ",openFileCnt - closeCnt, "." 
endif
...
local totalDir1Files = SCX_CPU1_FM_TotalFiles
totalExpected1 = SCX_CPU1_FM_TotalFiles - closeCnt
write "*** Total Files in directory before delete = ", SCX_CPU1_FM_TotalFiles
write "*** Total Files expected after delete = ", totalExpected1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DeleteAll DirName=testDir1
...
if (SCX_CPU1_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_find_event[2].num_found_messages = 1) THEN
    write "<*> Passed (2007.1.1) - Event message ", SCX_CPU1_find_event[2].eventid, " received"
    ut_setrequirements FM_200711, "P"
  else
    write "<!> Failed (2007.1.1) - Did not receive EM " & SCX_CPU1_find_event[2].eventid & " Either the wrong EM is expected or the wrong one was received. "
    ut_setrequirements FM_200711, "F"
  endif
else
...
write "*** Total Files in directory after delete = ", SCX_CPU1_FM_TotalFiles

if (totalExpected1 = SCX_CPU1_FM_TotalFiles) then
  write "<*> Passed (2007.1)- All Closed files were deleted"
  ut_setrequirements FM_20071, "P"
else
  write "<!> Failed (2007.1) - Expected ",totalExpected1, " files in the directory after delete. Found ",SCX_CPU1_FM_TotalFiles
  ut_setrequirements FM_20071, "F"
endif
...
write "*** Total Open Files = ", SCX_CPU1_FM_TotalOpenFiles

filesToClose = SCX_CPU1_FM_TotalOpenFiles
if (SCX_CPU1_FM_TotalOpenFiles > OS_MAX_NUM_OPEN_FILES) then
  filesToClose = OS_MAX_NUM_OPEN_FILES
endif

for j = 1 to filesToClose do
  if (SCX_CPU1_FM_OpenFileList[j].AppName = "TST_FM") then
    ut_setupevents "SCX", "CPU1", "TST_FM", TST_FM_CLOSEFILE_INF_EID, "INFO", 1

    cmdctr = SCX_CPU1_TST_FM_CMDPC + 1

    fullFileName = SCX_CPU1_FM_OpenFileList[j].FileName
    /SCX_CPU1_TST_FM_Close File=fullFileName 
...
if (UT_TW_Status = UT_Success) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> Passed (1003) - Expected event message ", SCX_CPU1_find_event[1].eventid," received."
    ut_setrequirements FM_1003, "P"
  else
...
write "*** Total Files in directory = ", SCX_CPU1_FM_TotalFiles
if (SCX_CPU1_FM_TotalFiles = 0) then
  write "<*> Passed (2007) - Directory Listing verifies that ALL files were deleted."
  ut_setrequirements FM_2007, "P"
else
  write "<!> Failed (2007) - Directory '", testDir1, "' contained ",SCX_CPU1_FM_TotalFiles, " files. Expected 0 files."
  ut_setrequirements FM_2007, "F"
endif
...
if (SCX_CPU1_FM_TotalFiles = 0) then
  ...
  write "<!> Failed (2007) - Directory '", testDir2, "' contained ",SCX_CPU1_FM_TotalFiles, " files. Expected 0 files."
  ut_setrequirements FM_2007, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_OPEN_FILES_PKT_ERR_EID,"ERROR", 1
...
if (UT_TW_Status = UT_Success) then
  ...
  if (SCX_CPU1_num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_GET_OPEN_FILES_PKT_ERR_EID
    ut_setrequirements FM_1004, "F"
  endif
else
...
write ""
write ";*********************************************************************"
write ";  End procedure scx_cpu1_fm_openfiles                                "
write ";*********************************************************************"
ENDPROC
