...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_NOOP_CMD_EID, "INFO", 1

ut_sendcmd "SCX_CPU1_FM_NOOP"
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1000;1003) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements FM_1000, "P"
  ut_setrequirements FM_1003, "P"
else
  write "<!> Failed (1000;1003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_NOOP_CMD_EID, "."
  ut_setrequirements FM_1000, "F"
  ut_setrequirements FM_1003, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_NOOP_PKT_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_NOOP_PKT_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
if (SCX_CPU1_FM_CMDPC = 0) then
  ;; Send a NOOP command
  /SCX_CPU1_FM_NOOP
  wait 5
endif

if (SCX_CPU1_FM_CMDEC = 0) then
  /raw {rawcmd}
  wait 5
endif

;; Setup for the expected event
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_RESET_CMD_EID, "DEBUG", 1

/SCX_CPU1_FM_RESETCTRS
wait 5

ut_tlmwait SCX_CPU1_FM_CMDPC, 0
if (UT_TW_Status = UT_Success) then
...
ut_tlmwait SCX_CPU1_FM_CMDEC, 0
if (UT_TW_Status = UT_Success) then
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1001) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_RESET_CMD_EID, "."
  ut_setrequirements FM_1001, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_RESET_PKT_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_RESET_PKT_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
page SCX_CPU1_FM_FREESPACE_TLM

write ";*********************************************************************"
write ";  Step 2.5: Send a valid Get Free Space command."
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_FREE_SPACE_CMD_EID, "DEBUG", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_GET_FREE_SPACE_CMD_EID, "."
  ut_setrequirements FM_1003, "F"
endif

;; Check the tlm
for i = 0 to FM_TABLE_ENTRY_COUNT-1 do
  if (p@SCX_CPU1_FM_FreeSpaceTBL[i].State = "Enabled") then
    ; This write statement will work on 64-bit systems
    ; if it is commented out, uncomment if needed
    ;write "Free space on '",SCX_CPU1_FM_FreeSpacePkt[i].Name, "' is ", %hex(SCX_CPU1_FM_FreeSpacePkt[i].Upper32)," and ",%hex(SCX_CPU1_FM_FreeSpacePkt[i].Lower32)
    ; 32-bit system does not have the Upper32 in the RDL
    write "Free space on '",SCX_CPU1_FM_FreeSpacePkt[i].Name, "' is ", %hex(SCX_CPU1_FM_FreeSpacePkt[i].Lower32)
  elseif (p@SCX_CPU1_FM_FreeSpaceTBL[i].State = "Disabled") then
    write "Table entry for '",SCX_CPU1_FM_FreeSpaceTBL[i].Name, "' is disabled."
  else
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_FREE_SPACE_PKT_ERR_EID,"ERROR",1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_GET_FREE_SPACE_PKT_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName},FM_SET_TABLE_STATE_CMD_EID, "INFO", 1

local newFSState = FM_TABLE_ENTRY_UNUSED

;; Use the first entry. Set its state to the opposite of what it is currently
if (p@SCX_CPU1_FM_FreeSpaceTBL[0].State = "Enabled") then
...
/SCX_CPU1_FM_SetTblState TBLEntry=0 NewState=newFSState
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_SET_TABLE_STATE_CMD_EID, "."
  ut_setrequirements FM_1003, "F"
endif
...
if (SCX_CPU1_FM_FreeSpaceTBL[0].State = newFSState) then
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_SET_TABLE_STATE_PKT_ERR_EID,"ERROR",1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_SET_TABLE_STATE_PKT_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_SET_TABLE_STATE_ARG_IDX_ERR_EID,"ERROR",1

errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_SetTblState TBLEntry=FM_TABLE_ENTRY_COUNT NewState=FM_TABLE_ENTRY_ENABLED
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_SET_TABLE_STATE_ARG_IDX_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_SET_TABLE_STATE_UNUSED_ERR_EID,"ERROR", 1

;; Find an UNUSED entry in the Free Space Table
local fsEntry=0
for i = 0 to FM_TABLE_ENTRY_COUNT-1 do
  if (SCX_CPU1_FM_FreeSpaceTBL[i].State = FM_TABLE_ENTRY_UNUSED) then
    fsEntry = i
    break
  endif
enddo
...
/SCX_CPU1_FM_SetTblState TBLEntry=fsEntry NewState=FM_TABLE_ENTRY_ENABLED
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_SET_TABLE_STATE_UNUSED_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_SET_TABLE_STATE_ARG_STATE_ERR_EID,"ERROR",1

errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_SetTblState TBLEntry=0 NewState=3
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_SET_TABLE_STATE_ARG_STATE_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
write ";*********************************************************************"
;; Delete the apps
/SCX_CPU1_ES_DeleteApp Application="TST_FM"
wait 2
/SCX_CPU1_ES_DeleteApp Application=FMAppName
wait 2
...
s ftp_file ("CF:0", FM_TABLE_FILENAME, FM_TABLE_FILENAME, hostCPU, "R")
wait 5

;; Clear the fm pages since new ASIST version displays multiple copies
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_SET_TABLE_STATE_TBL_ERR_EID,"ERROR",1

errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_SetTblState TBLEntry=1 NewState=FM_TABLE_ENTRY_ENABLED
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_SET_TABLE_STATE_TBL_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CC_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_CC_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
write ";*********************************************************************"
;ut_setupevents "SCX", "CPU1", {FMAppName}, FM_STARTUP_EID, "INFO", 1

/SCX_CPU1_ES_RESTARTAPP APPLICATION=FMAppName
wait 5
; 2.5.3 Work-around: Due to FM returning an error in termination 
;/SCX_CPU1_ES_DELETEAPP APPLICATION=FMAppName
;wait 5
;/SCX_CPU1_ES_DELETEAPP APPLICATION="TST_FM"
;wait 5
...
ut_tlmupdate SCX_CPU1_FM_CMDPC
...
write ";  validation. "
write ";*********************************************************************"
s scx_cpu1_fm_badtblloadfile
wait 5

write ";*********************************************************************"
write ";  Step 3.5: Load the table image created above to CPU1."
write ";*********************************************************************"
cmdCtr = SCX_CPU1_TBL_CMDPC + 1

ut_setupevents "SCX", "CPU1", "CFE_TBL", CFE_TBL_FILE_LOADED_INF_EID, "INFO", 1

s load_table("cpu1_fmbadtbl_ld_1",hostCPU)
...
ut_setupevents "SCX","CPU1",{FMAppName},FM_TABLE_VERIFY_EID, "INFO", 3
ut_setupevents "SCX","CPU1",{FMAppName},FM_TABLE_VERIFY_EMPTY_ERR_EID,"ERROR", 4

cmdCtr = SCX_CPU1_TBL_CMDPC + 1

/SCX_CPU1_TBL_VALIDATE INACTIVE VTABLENAME=freeSpaceTblName
...
if (UT_TW_Status = UT_Success) then
  write "<*> Passed - Validation Error Event Msg ",SCX_CPU1_find_event[2].eventid," Found!"
else
...
if (SCX_CPU1_find_event[3].num_found_messages = 1) then
  write "<*> Passed - FM Validation Error Event Msg ",SCX_CPU1_find_event[3].eventid," Found!"
else
...
if (SCX_CPU1_find_event[4].num_found_messages = 1) then
  write "<*> Passed - FM Validation Error Event Msg ",SCX_CPU1_find_event[4].eventid," Found!"
else
  write "<!> Failed - FM Validation Error Event Message ",FM_TABLE_VERIFY_EMPTY_ERR_EID," not received."
endif
...
write ""
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_fm_GenCmds                                  "
write ";*********************************************************************"
ENDPROC
