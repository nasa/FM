...
for file_index = 1 to CFE_ES_MAX_APPLICATIONS do
  if (SCX_CPU1_ES_ALE[file_index].ES_AL_AppName = FMAppName) then
    found_app1 = TRUE
  elseif (SCX_CPU1_ES_ALE[file_index].ES_AL_AppName = "TST_FM") then
    found_app2 = TRUE
  endif
enddo
...
if (found_app1 = FALSE) then
  ...
  ut_setupevents "SCX", "CPU1", {FMAppName}, FM_STARTUP_EID, "INFO", 2
  ...
  if (FM_CHILD_TASK_PRIORITY > fm_task_priority) then
    ...
    /SCX_CPU1_ES_StartApp Application=FMAppName APP_Entry_PT="FM_AppMain" APP_File_Name="/ram/fm.o" STACKSIZE=X'2000' PRIORITY=fm_task_priority RESTARTCPU
  endif
  ...
  /SCX_CPU1_TO_ADDPACKET Stream=stream2 Pkt_Size=x'0' Priority=x'0' Reliability=x'1' Buflimit=x'4'
  wait 5
  ...
  /SCX_CPU1_TO_ADDPACKET Stream=stream3 Pkt_Size=x'0' Priority=x'0' Reliability=x'1' Buflimit=x'4'
  wait 5
  ...
  /SCX_CPU1_TO_ADDPACKET Stream=stream4 Pkt_Size=x'0' Priority=x'0' Reliability=x'1' Buflimit=x'4'
  wait 5

  write "Sending command to add subscription for FM Free Space packet."
  /SCX_CPU1_TO_ADDPACKET Stream=stream5 Pkt_Size=x'0' Priority=x'0' Reliability=x'1' Buflimit=x'4'
  wait 5
endif
...
/SCX_CPU1_EVS_EnaAppEVTType Application=FMAppName DEBUG
...
if (found_app2 = FALSE) then
  ...
  write ";********************************************************************"
  ut_setupevents "SCX", "CPU1", "CFE_ES", CFE_ES_START_INF_EID, "INFO", 1
  ut_setupevents "SCX", "CPU1", "TST_FM", TST_FM_INIT_INF_EID, "INFO", 2
...
procterm:
write ";*********************************************************************"
write ";  End procedure scx_cpu1_fm_startfmapps "
write ";*********************************************************************"
ENDPROC
