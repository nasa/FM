PROC scx_cpu1_fm_filerename_basic
;*******************************************************************************
...
local fullOverwriteFileName = testDir1 & "/" & testFile2
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_InfoFileName = fullTestFile1Name) THEN
...
s scx_cpu1_fm_fileinfodisplay (fullTestFile2Name)
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (p@SCX_CPU1_FM_FileStatus = "Not Used") THEN
    ...
    write "<!> Failed - File Info Telemetry indicates a status of '",p@SCX_CPU1_FM_FIleStatus, "'. Expected 'Not Used'"
  endif
else
...
/SCX_CPU1_FM_FileRename Overwrite=0 File=fullTestFile1Name DestName=fullTestFile2Name
...
if (UT_TW_Status = UT_Success) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_RENAME_CMD_EID, "."
    ut_setrequirements FM_1003, "F"
  endif
else
...
if ( cmdctr = SCX_CPU1_FM_CMDPC ) then
  ...
  if ( SCX_CPU1_FM_InfoFileName = fullTestFile2Name ) THEN
...
s scx_cpu1_fm_fileinfodisplay (fullTestFile3Name)
...
/SCX_CPU1_FM_FileRename Overwrite=0 File=fullTestFile2Name DestName=fullTestFile3Name
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if ( SCX_CPU1_FM_InfoFileName = fullTestFile3Name ) THEN
...
local expOpenFiles = SCX_CPU1_FM_NumOpen + 1
cmdctr = SCX_CPU1_TST_FM_CMDPC + 1

/SCX_CPU1_TST_FM_Open File=fullTestFile3Name
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> (1003) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements FM_1003, "P"
  else
    write "<!> Failed (1003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",FM_GET_FILE_INFO_CMD_EID
    ut_setrequirements FM_1003, "F"
  ...
  if (p@SCX_CPU1_FM_FileStatus = "Open") THEN
    write "<*> Passed - File Info Telemetry indicates File is open"
  else
    write "<!> Failed - File Info Telemetry does not indicate File is open"
  endif

  if (SCX_CPU1_FM_NumOpen = expOpenFiles) THEN
    write "<*> Passed - HK Telemetry Reports number of open files correctly: " & SCX_CPU1_FM_NumOpen
  else
    write "<!> Failed - HK Telemetry Reports number of open files incorrectly. Expected ",expOpenFiles," rcv'd ",SCX_CPU1_FM_NumOpen
  endif
else
  write "<!> Failed (1003;2011) - File Info Command incremented the CMDEC"
  ut_setrequirements FM_1003, "F"
...
/SCX_CPU1_FM_FileRename Overwrite=0 File=fullTestFile3Name DestName=testDir2 & "/openSrc.tst"
...
local openFileSize = 0
cmdctr = SCX_CPU1_FM_CMDPC + 1

s scx_cpu1_fm_fileinfodisplay (openFileDestName)
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_FileStatus > 1) THEN
    ...
    openFileSize = SCX_CPU1_FM_InfoFileSize
  else
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_RENAME_TGT_EXIST_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_FileRename Overwrite=0 File=fullOverwriteFileName DestName=openFileDestName 
...
if (UT_TW_Status = UT_Success) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> Passed (1004) - Expected event message ", SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements FM_1004, "P"
  else
    write "<!> Failed - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_RENAME_TGT_EXIST_ERR_EID
...
write ";*********************************************************************"
write ";  Step 3.12:  File Info Command to Verify Dest File was not overwritten"
write ";  in Destination Directory."
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_FILE_INFO_CMD_EID, "DEBUG", 1
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  ;;Check the status of the file
  if (SCX_CPU1_FM_InfoFileSize = openFileSize) THEN
    write "<*> Passed - File Info Telemetry indicates that '",openFileDestName,"' was not overwritten"
  else
    write "<!> Failed - File Info Telemetry indicates that '",openFileDestName,"' was overwritten"
  endif
else
  write "<!> Failed (1003;2011) - File Info Command Failed."
  ut_setrequirements FM_1003, "F"
...
write ";*********************************************************************"
write ";  Step 3.13: Perform Rename File Command to Rename an existing "
write ";  Destination file. This should fail because the dest file is open."
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_RENAME_TGT_ISOPEN_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_FileRename Overwrite=1 File=fullOverwriteFileName DestName=openFileDestName 
...
write ";*********************************************************************"
write ";  Step 3.14:  Upload the original test file."
write ";*********************************************************************"
s ftp_file (uploadDir, testFile1, testFile1, hostCPU, "P")
wait 5

write ";*********************************************************************"
write ";  Step 3.15:  File Info Command to Verify File Exists "
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_FILE_INFO_CMD_EID, "DEBUG", 1
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_FileStatus > 1) THEN
    write "<*> Passed - File Info Telemetry indicates that '",fullTestFile1Name,"' exists"
    file1Size = SCX_CPU1_FM_InfoFileSize
  else
    write "<!> Failed - File Info Telemetry indicates that '",fullTestFile1Name,"' does not exist"
  endif
else
...
write ";*********************************************************************"
write ";  Step 3.16:  File Info Command to Verify File Exists "
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_FILE_INFO_CMD_EID, "DEBUG", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1

s scx_cpu1_fm_fileinfodisplay (fullOverwriteFileName)
...
write ";*********************************************************************"
write ";  Step 3.17: Perform Rename File Command to Rename an existing "
write ";  Source File"
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_RENAME_CMD_EID, "DEBUG", 1
...
/SCX_CPU1_FM_FileRename Overwrite=1 File=fullOverwriteFileName DestName=fullTestFile1Name

ut_tlmwait  SCX_CPU1_FM_CMDPC, {cmdctr}
if (UT_TW_Status = UT_Success) then
  write "<*> Passed (2005) - Rename Command with overwrite accepted."
  ut_setrequirements FM_2005, "P"
  ...
  write "<!> Failed (1003;2005) - Rename Command of an existing file rejected."
  ut_setrequirements FM_1003, "F"
  ut_setrequirements FM_2005, "F"
endif
...
write ";*********************************************************************"
write ";  Step 3.18:  File Info Command to Verify Source File does not exist."
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_FILE_INFO_CMD_EID, "DEBUG", 1
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  ;;Check the status of the file
  if (p@SCX_CPU1_FM_FileStatus = "Not Used") THEN
    write "<*> Passed - File Info Telemetry indicates that '",fullOverwriteFileName,"' does not exist"
  else
    write "<!> Failed - File Info Telemetry indicates that '",fullOverwriteFileName,"' still exists."
  endif
else
  write "<!> Failed (1003;2011) - File Info Command Failed."
  ut_setrequirements FM_1003, "F"
...
write ";*********************************************************************"
write ";  Step 3.19:  File Info Command to Verify Dest File was overwritten."
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_FILE_INFO_CMD_EID, "DEBUG", 1
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  ;;Check the status of the file
  if (SCX_CPU1_FM_InfoFileSize != file1Size) THEN
    write "<*> Passed - File Info Telemetry indicates that '",fullTestFile1Name,"' was overwritten as expected"
  else
    write "<!> Failed - File Info Telemetry indicates that '",fullTestFile1Name,"' was not overwritten"
  endif
else
  write "<!> Failed (1003;2011) - File Info Command Failed."
  ut_setrequirements FM_1003, "F"
...
write ""
write ";*********************************************************************"
write ";  End procedure scx_cpu1_fm_filerename_basic   "
write ";*********************************************************************"
ENDPROC
