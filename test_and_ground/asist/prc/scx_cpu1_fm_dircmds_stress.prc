PROC scx_cpu1_fm_dircmds_stress
;*******************************************************************************
...
write ";********************************************************************"
s scx_cpu1_fm_tableloadfile
wait 5

s ftp_file ("CF:0", "cpu1_fmdevtbl_ld_1", FM_TABLE_FILENAME, hostCPU, "P")
wait 5

s scx_cpu1_fm_startfmapps
wait 5
...
if (SCX_CPU1_FM_CMDPC = 0) AND (SCX_CPU1_FM_CMDEC = 0) THEN
  ...
  write "  CMDPC           = ",SCX_CPU1_FM_CMDPC
  write "  CMDEC           = ",SCX_CPU1_FM_CMDEC
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CREATE_DIR_CMD_EID, "DEBUG", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DirCreate DirName=testSourceDir

wait until ((SCX_CPU1_FM_CMDPC = cmdctr) OR (SCX_CPU1_FM_CMDEC = errcnt))
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements FM_1003, "P"
else
  write "<!> Failed (1003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_CREATE_DIR_CMD_EID, "."
  ut_setrequirements FM_1003, "F"
endif
...
/SCX_CPU1_FM_DirCreate DirName=testDestDir
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CREATE_DIR_PKT_ERR_EID, "ERROR", 1
...
ut_sendrawcmd "SCX_CPU1_FM", (rawcmd)

wait until ((SCX_CPU1_FM_CMDPC = cmdctr) OR (SCX_CPU1_FM_CMDEC = errcnt))
if (SCX_CPU1_FM_CMDEC = errcnt) then
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CREATE_DIR_SRC_ISDIR_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Did not receive EM ", FM_CREATE_DIR_SRC_ISDIR_ERR_EID
    ut_setrequirements FM_1004, "F"
  endif
else
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CREATE_DIR_SRC_DNE_ERR_EID, "ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DirCreate DirName= testSourceDir & forwardSlash & testFile
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Did not receive EM ", FM_CREATE_DIR_SRC_DNE_ERR_EID
    ut_setrequirements FM_1004, "F"
  endif
else  
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CREATE_DIR_OS_ERR_EID, "ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CHILDCMDEC + 1

/SCX_CPU1_FM_DirCreate DirName= ramDir & "/nonexist1/nonexist2/nonexist3/nonexist4"

wait until ((SCX_CPU1_FM_CMDPC = cmdctr) OR (SCX_CPU1_FM_CHILDCMDEC = errcnt))
if (SCX_CPU1_FM_CHILDCMDEC = errcnt) then
...
s scx_cpu1_fm_dirfiledisplay (fileSystem, ramDir & "/fm_creatstress_1.lst", ramDirPhys, "fm_creatstress_1.lst")
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CREATE_DIR_SRC_INVALID_ERR_EID, "ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DirCreate DirName=""
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Did not receive EM ",SCX_CPU1_find_event[1].eventid
  endif
else
...
/SCX_CPU1_FM_DirCreate DirName=testDirName
...
write ""
s scx_cpu1_fm_dirfiledisplay (ramDir,ramDir & "/fm_creatstress_2.lst",ramDirPhys,"fm_creatstress_2.lst")

FOR currentDis = 1 to SCX_CPU1_FM_NumFilesWritten DO  
  if (SCX_CPU1_FM_FileListEntry[currentDis].Name = testDirNameOnly) then
    write "<*> Passed - Directory found in file listing at file name number ",currentDis
    CharsInCreate_63 = "YES"
    goto EXITFOR1
  elseif (currentDis = SCX_CPU1_FM_NumFilesWritten) then
...
write "<**> Check the contents of ",ramDir," to verify the directory exists"
write ""
s scx_cpu1_fm_dirfiledisplay (ramDir,ramDir & "/fm_creatstress_3.lst",ramDirPhys,"fm_creatstress_3.lst")
...
write "<**> Check the contents of ",ramDir," to verify the directory exists"
write ""
s scx_cpu1_fm_dirfiledisplay (ramDir,ramDir & "/fm_creatstress_4.lst",ramDirPhys,"fm_creatstress_4.lst")
...
/SCX_CPU1_FM_DirCreate DirName=fullTestShortName
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_PKT_PKT_ERR_EID,"ERROR", 1
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_PKT_SRC_DNE_ERR_EID,"ERROR", 1

errcnt = SCX_CPU1_FM_CMDEC + 1

s scx_cpu1_fm_dirtlmdisplay (ramDir & "/imaginary/", 0,"Fail")

if (errcnt = SCX_CPU1_FM_CMDEC) then
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_PKT_SRC_ISDIR_ERR_EID,"ERROR", 1

errcnt = SCX_CPU1_FM_CMDEC + 1

s scx_cpu1_fm_dirtlmdisplay (fullOriginalTestFileName, 0,"Fail")
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_PKT_SRC_INVALID_ERR_EID,"ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

s scx_cpu1_fm_dirtlmdisplay ("", 0, "Fail")
...
;; Upload the Test file
s ftp_file (uploadDir2 & "FMLIMITSTEST", testFile2, testFile2, hostCPU, "P")

; 62 + null total path length 
;; Upload the Test file
s ftp_file (uploadDir3 & "FMLIMITSTEST", testFile2, testFile2, hostCPU, "P")

; 61 + null total path length 
;; Upload the Test file
s ftp_file (uploadDir4 & "FMLIMITSTEST", testFile2, testFile2, hostCPU, "P")
wait 5
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_DIR_PKT_CMD_EID, "DEBUG", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1

testDirName = longDirName1 & "FMLIMITSTEST"

s scx_cpu1_fm_dirtlmdisplay (testDirName, 0)

if (cmdctr = SCX_CPU1_FM_CMDPC) then
...
FOR currentDis = 1 to SCX_CPU1_FM_PktFiles DO
  if (SCX_CPU1_FM_DirList[currentDis].Name = testFile2) then
    write "<*> Passed - File found in file listing at file name number ", currentDis
    CharsInDirLstTlm_63 = "YES"
    goto EXITFOR4
  elseif (currentDis = SCX_CPU1_FM_PktFiles) then
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_DIR_PKT_CMD_EID, "DEBUG", 1
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_GET_DIR_PKT_WARNING_EID, "INFO", 2
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_find_event[2].num_found_messages = 1) THEN
    write "<*> Passed - Name length warning message ",SCX_CPU1_find_event[2].eventid, " received."
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_find_event[2].num_found_messages = 1) THEN
    write "<*> Passed - Name length warning message ",SCX_CPU1_find_event[2].eventid, " received as expected."
;; Commenting out the else since this works in 2.4.1.0
;;  else
;;    write "<!> Failed - Expected Name length warning message ", FM_GET_DIR_PKT_WARNING_EID, " NOT received."
  endif
...
FOR currentDis = 1 to SCX_CPU1_FM_PktFiles DO
  if (SCX_CPU1_FM_FileListEntry[currentDis].Name = testFile2) then
    write "<*> Passed - File found in file listing at file name number ", currentDis
    CharsInDirLstTlm_61 = "YES"
    goto EXITFOR6
  elseif (currentDis = SCX_CPU1_FM_PktFiles) then
...
s scx_cpu1_fm_dirtlmdisplay (fullTestShortName, 0)
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1003) - Event message ", SCX_CPU1_evs_eventid," received. Expected Event message ", FM_GET_DIR_PKT_CMD_EID, "."
  ut_setrequirements FM_1003, "F"
endif
...
/SCX_CPU1_FM_DirCreate DirName= testDestDir & "2"
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1003) - Event message ",SCX_CPU1_evs_eventid, " received"
  ut_setrequirements FM_1003, "P"
else
  write "<!> Failed (1003) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_CREATE_DIR_CMD_EID, "."
  ut_setrequirements FM_1003, "F"
...
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_COPY_OS_ERR_EID, "ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC
errcnt = SCX_CPU1_FM_CMDEC

;; Copy files until the Error Event occurs
FOR copyNumber = 1 to numFileCopies DO
  cmdctr = cmdctr + 1

  /SCX_CPU1_FM_FileCopy Overwrite=0 DestName= testDestDir & "2" & forwardSlash & "file" & copyNumber & ".tst" , File= fullOriginalTestFileName
...
s scx_cpu1_fm_dirtlmdisplay (testDestDir & "2", 0)
...
s scx_cpu1_fm_dirtlmdisplay (testDestDir & "2", fileOffset)
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_DELETE_ALL_CMD_EID, "DEBUG", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DeleteAll DirName= testDestDir & "2"
...
if (UT_TW_Status = UT_Success) then
  write "<*> - Event message ", SCX_CPU1_find_event[1].eventid, " received"
else
  write "<!> - Event message ", SCX_CPU1_evs_eventid," received. Expected Event message ", FM_DELETE_ALL_CMD_EID, "."
endif
...
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_PKT_ERR_EID,"ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

;; CPU1 is the default
rawcmd = "188CC00000860ECE"
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_SRC_DNE_ERR_EID,"ERROR", 1

errcnt = SCX_CPU1_FM_CMDEC + 1

s scx_cpu1_fm_dirfiledisplay (ramDir & "/imaginary/", testDestDir & forwardSlash & beginFileName & listFileNum & extension, downloadDir, beginFileName & listFileNum & extension, "Fail")
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) then
    write "<*> Passed - Event message ",SCX_CPU1_find_event[3].eventid," received."
    ut_setrequirements FM_1004, "P"
  else
    write "<!> Failed (1004) - Event message ", SCX_CPU1_find_event[3].eventid, " received."
    ut_setrequirements FM_1004, "F"
  endif
else
  write "<!> Failed (1004;1006) - Command Rejected Counter did not increment as expected."
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_SRC_ISDIR_ERR_EID,"ERROR", 1

errcnt = SCX_CPU1_FM_CMDEC

s scx_cpu1_fm_dirfiledisplay (fullOriginalTestFileName, testDestDir & forwardSlash & beginFileName & listFileNum & extension, downloadDir, beginFileName & listFileNum & extension, "Fail")
...
if (SCX_CPU1_FM_CMDEC > errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Event message ", FM_GET_DIR_FILE_SRC_ISDIR_ERR_EID, " NOT received."
    ut_setrequirements FM_1004, "F"
  endif
else
  write "<!> Failed (1004;1006) - Command Rejected Counter did not increment as expected."
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_OSCREAT_ERR_EID,"ERROR", 1

errcnt = SCX_CPU1_FM_CHILDCMDEC + 1

s scx_cpu1_fm_dirfiledisplay (testDestDir, badPath, "RAM:0/imaginary/", "file.tst","Fail")
...
if (SCX_CPU1_FM_CHILDCMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Event message ",FM_GET_DIR_FILE_OSCREAT_ERR_EID, " NOT received."
    ut_setrequirements FM_1004, "F"
  endif
else
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_TGT_ISDIR_ERR_EID,"ERROR", 1

errcnt = SCX_CPU1_FM_CMDEC + 1

s scx_cpu1_fm_dirfiledisplay (testDestDir, testDestDir & forwardSlash & "", downloadDir, "", "Fail" )
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Event message ",FM_GET_DIR_FILE_TGT_ISDIR_ERR_EID, " NOT received."
    ut_setrequirements FM_1004, "F"
  endif
else
  write "<!> Failed (1004;1005) - Command Rejected Counter did not increment as expected."
...
s scx_cpu1_fm_dirfiledisplay (testDestDir, "", ramDirPhys, "fm_dirlist.out")

ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_TGT_ERR_EID,"ERROR", 1
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) then
    ...
    write "<!> Failed - Event message ",SCX_CPU1_find_event[1].eventid," NOT received."
  endif
else
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_CMD_EID,"DEBUG", 1
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_SRC_INVALID_ERR_EID,"ERROR", 2

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

s scx_cpu1_fm_dirfiledisplay ("", testDestDir & "/dummyFile.dat", downloadDir, "dummyFile.dat","Fail")
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[2].num_found_messages = 1) THEN
    ...
    write "<!> Failed - Did not receive EM ", SCX_CPU1_find_event[2].eventid, " Either the wrong EM is expected or the wrong one was received. "
  endif
else
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_GET_DIR_FILE_WARNING_EID,"INFO", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1

s scx_cpu1_fm_dirfiledisplay (longDirName1 & "FMLIMITSTEST", testDestDir & forwardSlash & beginFileName & listFileNum & extension, downloadDir, beginFileName & listFileNum & extension)  ; 63 char + null
...
s scx_cpu1_fm_dirfiledisplay (longDirName2 & "FMLIMITSTEST/", testDestDir & forwardSlash & beginFileName & listFileNum & extension, downloadDir, beginFileName & listFileNum & extension)
...
s scx_cpu1_fm_dirfiledisplay (longDirName3 & "FMLIMITSTEST/", testDestDir & forwardSlash & beginFileName & listFileNum & extension, downloadDir, beginFileName & listFileNum & extension)
...
s scx_cpu1_fm_dirfiledisplay (fullTestShortName, testDestDir & forwardSlash & beginFileName & listFileNum & extension, downloadDir, beginFileName & listFileNum & extension)
...
s scx_cpu1_fm_dirfiledisplay (testDestDir,"",downloadDir,"defaultFile.dat")
...
/SCX_CPU1_FM_DirCreate DirName= longDirName1
...
/SCX_CPU1_FM_DirCreate DirName=longDirName2
...
/SCX_CPU1_FM_DirCreate DirName=longDirName3
...
s scx_cpu1_fm_dirfiledisplay (testDestDir, longDirName1 & "/LONG111.TST", uploadDir2, "/LONG111.TST")
...
write ""
s scx_cpu1_fm_dirfiledisplay (longDirName1, ramDir & "/fm_dlfstress_3.lst", ramDirPhys, "fm_dlfstress_3.lst")

FOR currentDis = 1 to SCX_CPU1_FM_NumFilesWritten DO
  if (SCX_CPU1_FM_FileListEntry[currentDis].Name = "LONG111.TST") then
    write "<*> Passed - File found in file listing at file name number ", currentDis
...
s scx_cpu1_fm_dirfiledisplay (testDestDir, longDirName2 & "/LONG222.TST", uploadDir3, "/LONG222.TST")
...
s scx_cpu1_fm_dirfiledisplay (testDirName, ramDir & "/fm_dlfstress_4.lst", ramDirPhys, "fm_dlfstress_4.lst")

FOR currentDis = 1 to SCX_CPU1_FM_NumFilesWritten DO
  if (SCX_CPU1_FM_FileListEntry[currentDis].Name = "LONG222.TST") then
    write "<*> Passed - File found in file listing at file name number ", currentDis
    CharsInDirLstFile_File_62 = "YES"
    goto EXITFOR11
  elseif (currentDis = SCX_CPU1_FM_NumFilesWritten) then
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName},FM_GET_DIR_FILE_CMD_EID, "DEBUG", 1
ut_setupevents "SCX", "CPU1", {FMAppName},FM_GET_DIR_FILE_OS_ERR_EID,"ERROR", 2

cmdctr = SCX_CPU1_FM_CMDPC + 1

s scx_cpu1_fm_dirfiledisplay (testDestDir, longDirName3 & "/LONG333.TST", uploadDir4, "/LONG333.TST")
...
s scx_cpu1_fm_dirfiledisplay (testDirName, ramDir & "/fm_dlfstress_5.lst", ramDirPhys, "fm_dlfstress_5.lst")

FOR currentDis = 1 to SCX_CPU1_FM_NumFilesWritten DO
  if (SCX_CPU1_FM_FileListEntry[currentDis].Name = "LONG333.TST") then
    write "<*> Passed - File found in file listing at file name number ", currentDis
...
s scx_cpu1_fm_dirfiledisplay (testDestDir, fileSystem & "/S", ramDirPhys, "/S")
...
s scx_cpu1_fm_dirfiledisplay (testDestDir & "2", testDestDir & forwardSlash & beginFileName & listFileNum & extension, downloadDir, beginFileName & listFileNum & extension)
...
if (cmdctr <= SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> Passed (1003;3002.1) - Event message ", SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements FM_1003, "P"
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_DELETE_DIR_PKT_ERR_EID, "ERROR", 1
...
if ( SCX_CPU1_FM_CMDEC = errcnt ) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed - Did not receive EM ", SCX_CPU1_find_event[1].eventid
  endif
else
  write "<!> Failed (1002;1004) - Command Rejected Counter did not increment as expected."
  ut_setrequirements FM_1002, "F"
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_DELETE_DIR_SRC_DNE_ERR_EID, "ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DirDelete DirName= ramDir & "/imaginary/"
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_DELETE_DIR_SRC_ISDIR_ERR_EID, "ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DirDelete DirName=fullOriginalTestFileName
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_DELETE_DIR_SRC_INVALID_ERR_EID, "ERROR", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DirDelete DirName=""
...
/SCX_CPU1_FM_DirCreate DirName= longDirName1 & "FMLIMITTEST1"
...
/SCX_CPU1_FM_DirCreate DirName= longDirName2 & "FMLIMITTEST2"
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CREATE_DIR_CMD_EID, "DEBUG", 1
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_CREATE_DIR_OS_ERR_EID, "ERROR", 2

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DirCreate DirName= longDirName3 & "FMLIMITTEST3"
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_DELETE_DIR_CMD_EID, "DEBUG", 1

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1

/SCX_CPU1_FM_DirDelete DirName= longDirName1 & "FMLIMITTEST1"
...
s scx_cpu1_fm_dirfiledisplay (ramDir, ramDir & "/fm_delstress_2.lst", ramDirPhys, "fm_delstress_2.lst")

FOR currentDis = 1 to SCX_CPU1_FM_NumFilesWritten DO
  if ( SCX_CPU1_FM_FileListEntry[currentDis].Name = ("THISISREALLYLONGDIRECTORYNAMEFORTHETESTINGOFFM" & "FMLIMITTEST1")) then
...
/SCX_CPU1_FM_DirDelete DirName= longDirName2 & "FMLIMITTEST2/"
...
s scx_cpu1_fm_dirfiledisplay (ramDir, ramDir & "/fm_delstress_3.lst", ramDirPhys, "fm_delstress_3.lst")

FOR currentDis = 1 to SCX_CPU1_FM_NumFilesWritten DO
  if ( SCX_CPU1_FM_FileListEntry[currentDis].Name = ("THISIREALLYLONGDIRECTORYNAMEFORTHETESTINGOFFM" & "FMLIMITTEST1") ) then
    write "<!> Failed - File found in file listing at file name number ", currentDis
    CharsInDelete_62 = "NO"
    goto EXITFOR14
  elseif (currentDis = SCX_CPU1_FM_NumFilesWritten) then
...
/SCX_CPU1_FM_DirDelete DirName= longDirName3 & "FMLIMITTEST3/"
...
s scx_cpu1_fm_dirfiledisplay (ramDir, ramDir & "/fm_delstress_4.lst", ramDirPhys, "fm_delstress_4.lst")

FOR currentDis = 1 to SCX_CPU1_FM_NumFilesWritten DO
  if (SCX_CPU1_FM_FileListEntry[currentDis].Name = ("THISREALLYLONGDIRECTORYNAMEFORTHETESTINGOFFM" & "FMLIMITTEST1")) then
    write "<!> Failed - File found in file listing at file name number ", currentDis
    CharsInDelete_61 = "NO"
    goto EXITFOR15
  elseif (currentDis = SCX_CPU1_FM_NumFilesWritten) then
...
/SCX_CPU1_FM_DirDelete DirName=fullTestShortName
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1003) - Event message ", SCX_CPU1_evs_eventid," received. Expected Event message ", FM_DELETE_DIR_CMD_EID, "."
  ut_setrequirements FM_1003, "F"
endif
...
write ";*********************************************************************"
s scx_cpu1_fm_dirfiledisplay (testDestDir & "2", testDestDir & "2/fm_dest2.lst", ramDirPhys, "fm_dest2.lst")

;; Check the number of files contained in the directory
if (SCX_CPU1_FM_TotalFilesInDir = 0) then
  ;; Upload the Test file
  s ftp_file (uploadDestDir & "2", testFile, testFile, hostCPU,"P")
  wait 5
endif
...
write ";*********************************************************************"
ut_setupevents "SCX","CPU1",{FMAppName},FM_DELETE_DIR_EMPTY_ERR_EID,"ERROR", 1

errcnt = SCX_CPU1_FM_CHILDCMDEC + 1

/SCX_CPU1_FM_DirDelete DirName= testDestDir & "2"

ut_tlmwait SCX_CPU1_FM_CHILDCMDEC, {errcnt}
if (UT_TW_Status = UT_Success) then
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," received. Expected Event message ", FM_DELETE_ALL_CMD_EID, "."
endif
...
s scx_cpu1_fm_clearallpages

write ""
write ";*********************************************************************"
write ";  End procedure scx_cpu1_fm_dircmds_stress                           "
write ";*********************************************************************"
ENDPROC
