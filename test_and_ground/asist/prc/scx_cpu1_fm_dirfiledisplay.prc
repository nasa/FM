proc scx_cpu1_fm_dirfiledisplay (directory, filename, downloadDir, DLfilename, expResult, statFlag)
;==============================================================================
;
...
ut_setupevents "SCX", "CPU1", "FM", FM_GET_DIR_FILE_CMD_EID, "DEBUG", 5

cmdctr = SCX_CPU1_FM_CMDPC + 1
errcnt = SCX_CPU1_FM_CMDEC + 1
childCmdCtr = SCX_CPU1_FM_CHILDCMDPC + 1

/SCX_CPU1_FM_DirListFile DirName=directory File=filename GetStatFlag=statFlag
...
if ((SCX_CPU1_FM_CMDPC = cmdctr) AND (SCX_CPU1_FM_CHILDCMDPC = childCmdCtr)) then
  ...
  if (expResult = "Pass") then
    ...
    if (SCX_CPU1_find_event[5].num_found_messages = 1) then
      write "<*> Passed - Event message ", SCX_CPU1_find_event[5].eventid," received"
    else
...
if (actualResult = "Pass") then
  ...
  if (SCX_CPU1_find_event[5].num_found_messages = 1) then
    ...
    tokenType = %token(numFilesWritEM,SCX_CPU1_find_event[5].event_txt)

    DO UNTIL tokenType = "INTEGER"
      tokenType = %token(numFilesWritEM)
    ENDDO

    maxToDis = SCX_CPU1_FM_NumFilesWritten
  ...
  write "  Directory Name          = ", SCX_CPU1_FM_DirNameInFile
  write "  Total Files in Dir      = ", SCX_CPU1_FM_TotalFilesInDir
  write "  Number of Files Written = ", SCX_CPU1_FM_NumFilesWritten
  FOR currentDis = 1 to maxToDis DO
    if (currentDis = 1) then
      write ""
      write " File Listing: "
    endif
    write "  File " & currentDis & " Name           = ",SCX_CPU1_FM_FileListEntry[currentDis].Name
    write "  File " & currentDis & " Size           = ",SCX_CPU1_FM_FileListEntry[currentDis].FileSize
    write "  File " & currentDis & " Last Mod Time  = ",SCX_CPU1_FM_FileListEntry[currentDis].LastModTime
    write "  File " & currentDis & " Permissions    = ",SCX_CPU1_FM_FileListEntry[currentDis].FilePerms
  ENDDO
endif
