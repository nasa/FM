PROC scx_cpu1_fm_dircmds_basic
;*******************************************************************************
...
;		FM shall not update the following values:
;			b) file size in bytes of each file
;			c) last modification time of each file
;			e) Mode (permissions), as a 4-byte value, of each file
...
s scx_cpu1_fm_dirfiledisplay (parentDir, listDir & forwardSlash & listFilePre & filenum & listFileExt, downloadDir, listFilePre & filenum & listFileExt,"Pass")
...
if (SCX_CPU1_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_find_event[5].num_found_messages = 1) THEN
    write "<*> Passed (1003;3002.1) - Event message ",SCX_CPU1_find_event[5].eventid, " received"
    ut_setrequirements FM_1003, "P"
...
s scx_cpu1_fm_dirfiledisplay (testDir, listDir & forwardSlash & listFilePre & filenum & listFileExt, downloadDir, listFilePre & filenum & listFileExt,"Fail")
...
if (SCX_CPU1_FM_CMDEC = errcnt) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Event message ", FM_GET_DIR_FILE_SRC_DNE_ERR_EID, " NOT received."
...
s scx_cpu1_fm_dirfiledisplay (parentDir, "", downloadDir, listFilePre & filenum & listFileExt)
...
s scx_cpu1_fm_dirtlmdisplay (parentDir, 0)
...
s scx_cpu1_fm_dirtlmdisplay (testDir, 0, "Fail")
...
if (errcnt = SCX_CPU1_FM_CMDEC) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> Passed (1004) - Event message ", SCX_CPU1_find_event[3].eventid, " received"
    ut_setrequirements FM_1004, "P"
  else
    write "<!> Failed (1004) - Event message ", FM_GET_DIR_PKT_SRC_DNE_ERR_EID, " NOT received."
    ut_setrequirements FM_1004, "F"
  endif
else
...
s scx_cpu1_fm_dirfiledisplay (parentDir, listDir & forwardSlash & listFilePre & filenum & listFileExt, downloadDir, listFilePre & filenum & listFileExt)
...
if (SCX_CPU1_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_find_event[5].num_found_messages = 1) THEN
    ...
    FOR currentDis = 1 to SCX_CPU1_FM_NumFilesWritten DO
      if ( SCX_CPU1_FM_FileListEntry[currentDis].Name = testDirName ) then
...
s scx_cpu1_fm_dirfiledisplay (testDir, listDir & forwardSlash & listFilePre & filenum & listFileExt, downloadDir, listFilePre & filenum & listFileExt)
...
if (SCX_CPU1_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_FM_TotalFilesInDir = 0) THEN
    ...
    write "<!> Failed (3000;3002) - Directory Created, but file indicates wrong number of files: " & SCX_CPU1_FM_TotalFilesInDir
    ut_setrequirements FM_3000,   "F"
    ut_setrequirements FM_3002,   "F"
  endif
else
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  FOR currentDis = 1 to FM_DIR_LIST_PKT_ENTRIES DO
    if (SCX_CPU1_FM_DirList[currentDis].name = testDirName) then
...
s scx_cpu1_fm_dirtlmdisplay (testDir, 0)
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_TotalFiles = 0) THEN
    ...
    write "<!> Failed (3000;3003) - Directory Created, but Tlm indicates wrong number of files: ", SCX_CPU1_FM_TotalFiles
    ut_setrequirements FM_3000,   "F"
    ut_setrequirements FM_3003,   "F"
  endif
else
...
if (SCX_CPU1_FM_DirName = SCX_CPU1_FM_DirNameInFile) THEN
...
if (SCX_CPU1_FM_TotalFiles = SCX_CPU1_FM_TotalFilesInDir) THEN
  if (SCX_CPU1_FM_TotalFiles = 0) THEN
    ...
    write "<!> Failed (3000;3002;3002.1;3003) - Total Number of Files in Directory match, but indicate wrong number of files: ", SCX_CPU1_FM_TotalFiles
    ut_setrequirements FM_3000,   "F"
    ut_setrequirements FM_3002,   "F"
    ut_setrequirements FM_3002_1, "F"
    ut_setrequirements FM_3003,   "F"
...
if (SCX_CPU1_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_FM_FileListEntry[1].Name = testFile) THEN
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_DirList[1].Name = testFile) THEN
...
if (SCX_CPU1_FM_TotalFiles = SCX_CPU1_FM_TotalFilesInDir) THEN
  if (SCX_CPU1_FM_TotalFiles = 1) THEN
    write "<*> Passed (3002;3002.1;3003) - File and Telemetry Total Number of Files in Directory match"
    ut_setrequirements FM_3002,   "P"
    ut_setrequirements FM_3002_1, "P"
    ut_setrequirements FM_3003,   "P"
  else
    write "<!> Failed (3002;3002.1;3003) - Total Number of Files in Directory match, but indicate wrong number of files: " & SCX_CPU1_FM_TotalFiles
    ut_setrequirements FM_3002,   "F"
    ut_setrequirements FM_3002_1, "F"
    ut_setrequirements FM_3003,   "F"
  endif
...
if (SCX_CPU1_FM_NumFilesWritten > 0) THEN
  if (SCX_CPU1_FM_DirList[1].Name = SCX_CPU1_FM_FileListEntry[SCX_CPU1_FM_DirOffset+1].Name) THEN
...
/SCX_CPU1_FM_DirDelete DirName=testDir
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_DELETE_DIR_EMPTY_ERR_EID, "."
  ut_setrequirements FM_1004, "F"
endif
...
if (SCX_CPU1_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_find_event[5].num_found_messages = 1) THEN
    write "<*> Passed (3002.1) - Event message ",SCX_CPU1_find_event[5].eventid, " received"
    ut_setrequirements FM_1003, "P"
    ut_setrequirements FM_3002_1, "P"
  else
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
    write "<*> Paased (1003) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
    ut_setrequirements FM_1003, "P"
  else
    write "<!> Failed (1003) - Event message ", FM_GET_DIR_PKT_CMD_EID, " NOT received."
    ut_setrequirements FM_1003, "F"
...
if (SCX_CPU1_FM_TotalFiles = SCX_CPU1_FM_TotalFilesInDir) THEN
  if (SCX_CPU1_FM_TotalFiles = 1) THEN
    ...
    write "<!> Failed (3001.1;3002;3002.1;3003) - Total Number of Files in Directory match, but indicate wrong number of files: ", SCX_CPU1_FM_TotalFiles
    ut_setrequirements FM_3001_1, "F"
    ut_setrequirements FM_3002,   "F"
    ut_setrequirements FM_3002_1, "F"
    ut_setrequirements FM_3003,   "F"
...
/SCX_CPU1_FM_Delete File = fullTestFileName
...
if (SCX_CPU1_FM_CMDPC = cmdctr) then
  ...
  if (SCX_CPU1_FM_TotalFilesInDir = 0) THEN
    ...
    write "<!> Failed (2008) - File still reported as present in file; SCX_CPU1_FM_TotalFilesInDir: ", SCX_CPU1_FM_TotalFilesInDir
    ut_setrequirements FM_2008, "F"
  endif
else
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (SCX_CPU1_FM_TotalFiles = 0) THEN
    ...
    write "<!> Failed (2008) - File still reported as present in telemetry; SCX_CPU1_FM_TotalFiles: ", SCX_CPU1_FM_TotalFiles
    ut_setrequirements FM_2008, "F"
  endif
else
...
s scx_cpu1_fm_dirtlmdisplay (ramDir, 0, "Pass", 1)
...
s scx_cpu1_fm_dirtlmdisplay (ramDir, 0, "Pass", 0)

if ((SCX_CPU1_FM_DirList[1].FileSize = 0) AND ;;
    (SCX_CPU1_FM_DirList[1].LastModTime = 0) AND ;;
    (SCX_CPU1_FM_DirList[1].FilePerms = 0)) then
...
s scx_cpu1_fm_dirfiledisplay (ramDir,ramDir & "/dirfiletrue.lst",downloadDir,"dirfiletrue.lst","Pass",1)
...
;; Send the command again with flag set to False
s scx_cpu1_fm_dirfiledisplay (ramDir,ramDir & "/dirfilefalse.lst",downloadDir,"dirfilefalse.lst","Pass",0)

if ((SCX_CPU1_FM_FileListEntry[1].FileSize = 0) AND ;;
    (SCX_CPU1_FM_FileListEntry[1].LastModTime = 0) AND ;;
    (SCX_CPU1_FM_FileListEntry[1].FilePerms = 0)) then
...
write ";*********************************************************************"
write ";  End procedure scx_cpu1_fm_dircmds_basic                            "
write ";*********************************************************************"
ENDPROC
