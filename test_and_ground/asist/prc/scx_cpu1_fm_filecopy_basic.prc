PROC scx_cpu1_fm_filecopy_basic
;*******************************************************************************
...
/scx_cpu1_FM_DirCreate DirName=testDirSource
...
/scx_cpu1_FM_DirCreate DirName=testDirDest
...
s scx_cpu1_fm_fileinfodisplay (fullTestFileCopy1Name)
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {FMAppName}, FM_COPY_TGT_ISDIR_ERR_EID, "ERROR", 2

errcnt = SCX_CPU1_FM_CMDEC + 1

/scx_cpu1_FM_FileCopy Overwrite=0 File=fullOriginalTestFileName DestName= testDirDest & "/"
...
if (UT_TW_Status = UT_Success) then
  ...
  if (SCX_CPU1_find_event[2].num_found_messages = 1) THEN
    ...
    write "<!> Failed (1004) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ", FM_COPY_TGT_ISDIR_ERR_EID, "."
...
s scx_cpu1_fm_fileinfodisplay (fullTestFileCopy2Name)
...
/scx_cpu1_FM_FileCopy Overwrite=0 File=fullOriginalTestFileName DestName=fullTestFileCopy2Name
...
if (cmdctr = SCX_CPU1_FM_CMDPC) then
  ...
  if (scx_cpu1_FM_InfoFileName = fullTestFileCopy2Name) THEN
...
local filesOpen = scx_cpu1_FM_NumOpen + 1
...
if (scx_cpu1_FM_NumOpen = filesOpen) THEN
  write "<*> Passed - HK Telemetry Reports number of open files correctly: ", scx_cpu1_FM_NumOpen
else
  write "<!> Failed - The number of open files is incorrect. Expected ",filesOpen," TLM reports ",scx_cpu1_FM_NumOpen
endif
...
/scx_cpu1_FM_FileCopy Overwrite=0 File=fullOriginalTestFileName DestName= testDirDest & "/openSrc.tst"
...
s scx_cpu1_fm_fileinfodisplay (testDirDest & "/openSrc.tst")
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed - Close File command for '", fullOriginalTestFileName, "'. CMDPC = ",SCX_CPU1_TST_FM_CMDPC,"; Expected ",cmdctr
endif
...
/scx_cpu1_FM_FileCopy Overwrite=1 File=fullOriginalTestFileName DestName=fullTestFileCopy2Name
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_fm_filecopy_basic                                    "
write ";*********************************************************************"
ENDPROC
